...
__global__ void propagate(
    ...
    for (int k=idx; k<NPar; k+=stride){
        ...
        int nlayer_i = 0;                                   // Initial layer label
        int nlayer = 0;                                     // temporary layer lable
        ...
        while (1){
            ...
            nlayer = whichlayer(xPar, rCir, lm);
            if ( nlayer>=0 && nlayer < Nm) {
        ...
        for (int i=0; i<TN; i++){
            ...
            while (iterateFlag & (ite<nite)) {
                if ( !inLayerFlag ) {
                    ...
                    nlayer = whichlayer(xTmp, rCir, lm);
                    
                    if ( nlayer==nlayer_i ) {
                        // Case 2.1 Walker does not encounter the cell membrane
                        xt[0]=xTmp[0]; xt[1]=xTmp[1]; xt[2]=xTmp[2];
                        iterateFlag=false; inLayerFlag=true;
                    } else if (nlayer < nlayer_i) {
                        // Case 2.2 Walker encounters the inner cell membrane
                        // Similar to elastic collision in ECS
                        xCirTmp[0] = 0.5;
                        xCirTmp[1] = 0.5;
                        // rCirTmp = rCir + nlayer*lm;
                        ...
                        // Case 2.3 Walker encounters the outer cell membrane
                        // Similar to elastic collision in ICS
                        xCirTmp[0] = 0.5;
                        xCirTmp[1] = 0.5;
                        // rCirTmp = rCir + (nlayer+1)*lm;
                        ...
                        // Case 2.3.1 Renew the step for the elastic collision
                        xt[0]=xCollision[0]; xt[1]=xCollision[1]; xt[2]=xCollision[2];
                        iterateFlag=false;
                        
                        // Case 2.3.2 Cancel this step and choose another direction if bouncing twice
                        nlayer = whichlayer(xt, rCir, lm);
                        if ( nlayer!=nlayer_i ) {
                            xt[0]=xTmp[0]; xt[1]=xTmp[1]; xt[2]=xTmp[2];
                            iterateFlag=true; ite++;
            ...
            if ( (i%Tstep) ==0 ) { // Save moment tensor for dx^2 and dx^4, and signal for the b-table
                ...
                if ( inLayerFlag ) { atomAdd(&NParICS[tidx],1); }
...
int main(int argc, char *argv[]) {
    ...
    propagate<<<numBlocks, blockSize>>>(devState.data().get(), 
                                        ...
                                        d_NParBin.data().get(),
                                        d_sig.data().get(), 
                                        TN, NPar, res, step, stepz, Nbtab, NDelta, 
                                        rCir, lm, Nm, 
                                        d_btab.data().get(), 
                                        d_DELdel.data().get(),
                                        dt);
